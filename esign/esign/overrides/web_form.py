...
def on_update_esign_document(doc, method):
    ...
    if (
        web_form.esign_submit_on_response
        and doc.docstatus == 0
...
def accept(web_form, data):
    ...
    if data.name and frappe.form_dict.get("key"):
        try:
            ...
            validate_print_permission(doc)
        except (frappe.PermissionError, frappe.exceptions.LinkExpired) as e:
    ...
    if doc.name:
        # For eSign forms with valid key, always use ignore_permissions
        doc.save(ignore_permissions=True)
    else:
        # insert
