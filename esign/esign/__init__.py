...
def send_esign_email(
    ...
    if esign_link not in email_content:
        ...
        <div style="margin: 20px 0; padding: 15px; background-color: #f8f9fa; border-left: 4px solid #007bff; border-radius: 4px;">
            <h4 style="margin: 0 0 10px 0; color: #007bff;">eSign Required</h4>
            <p style="margin: 0 0 10px 0;">Please click the link below to review and sign the document:</p>
            <a href="{esign_link}" 
    ...
    try:
        ...
        doc.add_comment(
            "Comment", 
            _("eSign request sent to {0}").format(", ".join(recipients_list))
        )
        
        frappe.msgprint(
            _("eSign request sent successfully to {0}").format(", ".join(recipients_list)),
            title=_("Email Sent"),
            indicator="green"
        )
        ...
        frappe.log_error(
            title=f"eSign: Failed to send email for {doctype} {name}",
            message=str(e)
        )
